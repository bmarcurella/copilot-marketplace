...
Cowork plugins aren't installed from this marketplace — they're M365 app packages. Build, then upload:

```powershell
cd cowork-plugins/customer-architect
Compress-Archive -Path manifest.json, color.png, outline.png, skills -DestinationPath ../../dist/customer-architect.zip -Force
```

Then upload the `.zip` in **Cowork → Customize → Plugins → Upload plugin**, or attach it to a
[GitHub Release](../../releases) for others.
...
- **Library** items are standalone and inert here; bundle them into a plugin or copy into a project's `.github/`.
- Built `.zip` artifacts go to `dist/` (gitignored) and ship via GitHub Releases.
